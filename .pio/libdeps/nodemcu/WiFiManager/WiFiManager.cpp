...
boolean WiFiManager::startConfigPortal(char const *apName, char const *apPassword)
{
  ...
  connect = false;
  setupConfigPortal();
  portalStartTime = millis(); // Bắt đầu đếm thời gian
  while (1)
  {
    // Serial.println("Waiting for configuration...");
    //  Nếu đã quá 5 phút (300000 ms) thì restart ESP
    if (millis() - portalStartTime > 300000)
    { // 5 phút
      DEBUG_WM(F("⏰ Timeout 5 phút - Restart ESP8266..."));
      ESP.restart();
    }
    // check if timeout
...
void WiFiManager::resetSettings()
{
  DEBUG_WM(F("⚠ Resetting WiFi settings..."));

#if defined(ESP8266)
  // Ngắt kết nối và xóa thông tin Wi-Fi trong flash của SDK
  WiFi.disconnect(true); // Tham số true = xóa luôn SSID/PASS trong flash
  delay(200);

  // Đảm bảo không tự động kết nối lại
  WiFi.mode(WIFI_OFF);
  delay(200);

#elif defined(ESP32)
  // Ngắt kết nối và quên mạng
  WiFi.disconnect(true, true); // Tham số 1 = ngắt kết nối, 2 = quên mạng đã lưu
  delay(200);
  ...
  if (err == ESP_OK)
  {
    DEBUG_WM(F("NVS erased successfully."));
  }
  else
  {
    DEBUG_WM(F("NVS erase failed."));
  }
  nvs_flash_init();
  WiFi.mode(WIFI_OFF);
  delay(200);
  ...
  DEBUG_WM(F("WiFi settings cleared."));
}
